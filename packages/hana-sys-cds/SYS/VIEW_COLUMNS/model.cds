...
@title : 'Available view columns'
entity SYS.VIEW_COLUMNS {
  @title : 'Schema name'
  ...
  VIEW_OID : Integer64;
  @title : 'View column name'
  COLUMN_NAME : String(256);
  @title : 'Ordinal position of the view column'
  POSITION : Integer;
  @title : 'Data type ID'
  DATA_TYPE_ID : hana.SMALLINT;
  @title : 'Data type name'
  DATA_TYPE_NAME : String(16);
  @title : 'Offset of the column in a record'
  OFFSET : hana.SMALLINT;
  @title : 'Number of chars for character types; max number of digits for numeric types; number of chars for datetime types; number of bytes for LOB types'
  LENGTH : Integer;
  ...
  @title : 'Type of compression: ''NONE''/''DEFAULT''/''PREFIXED''/''SPARSE''/''CLUSTERED''/''INDIRECT''/''RLE'''
  ...
  GENERATED_ALWAYS_AS : String(1000);
  @title : 'Specifies if column has fuzzy search index: ''TRUE''/''FALSE'''
  FUZZY_SEARCH_INDEX : String(5);
  @title : 'Specifies fuzzy search mode'
  FUZZY_SEARCH_MODE : String(16);
  @title : 'Specifies memory threshold in bytes for LOB types'
  MEMORY_THRESHOLD : Integer;
  @title : 'Specifies unit of column data loading: ''TABLE''/''COLUMN''/''PAGE'''
  LOAD_UNIT : String(1);
  @title : '''ALWAYS AS'' if the column is generation column, ''ALWAYS AS IDENTITY''/''BY DEFAULT AS IDENTITY'' if the column is an identity column whose values are always generated or generated by default, NULL otherwise'
  GENERATION_TYPE : String(1);
  ...
  @title : 'Persistent Memory preference does not apply to view columns'
  PERSISTENT_MEMORY : String(1);
  @title : 'Comma separated list of ranges of user specified logical NUMA node indexes'
  NUMA_NODE_INDEXES : String(1);
};
